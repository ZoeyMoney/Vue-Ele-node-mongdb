        ...
        <el-card class="box-card box-input">
          ...
          <div class="text item">
            <el-form :inline="true" :model="formData" class="demo-form-inline">
              ...
              <div class="btn">
                <el-button type="info" @click="add(formData._id)">修改</el-button>
              </div>
            </el-form>
          </div>
        </el-card>
...
export default {
  ...
  data () {
    return {
      ...
      formInline: {
        ...
        textarea:'',
        Email: '',
        User:sessionStorage.getItem(config.KEY.CACHE_LOGIN_NAME),
        date:new Date(),
      },
      one_ge:false,
      formData:{
    ...
    handleSelects(key){
      switch (key) {
        case '1-1':
          ...
          break;
        case '1-2':
          this.formData = JSON.parse(localStorage.UserName)[0]
          this.one_ge = true;
          break;
        case '1-3':
  ...
  //  个人反馈提交
    info(){
      ...
      if (!myreg.test(this.formInline.phone)){
        this.$message({
          message: '手机号格式错误',
          type: 'warning'
        });
      ...
      if (!mEmail.test(this.formInline.Email)){
        this.$message({
          message: '请输入有效的邮箱',
          type: 'warning'
        });
  ...
  //  个人信息保存
    add(id){
      ...
      this.axios.post(`UserNameUpdate/${id}`,this.formData).then(res=>{
        if (res.status === 200){
